...
Verification-first sub-issue of #563: **confirm** the upstream training
...
- **Applied on all paths** — the upstream training-label producer (`output: [recommend]`)
  ...
  (the upstream training code). `volumeRecommend` enters via `Math.min`, so it can
  only lower a score; at `-1` it pins the output to `-1`. No bypass exists.
- **Biting** — driving the real upstream `profitRecommend`/`BUDGET_DOLLARS` exports,
...
- **Units correct** — the upstream training stores prices in cents (the upstream
  training code), so `averagePV / 100` yields dollars, directly comparable to
...
tracked in GRQ-validation while the guard lives upstream. No upstream PR is
...
Empirical confirmation, produced by importing the **real** upstream
...
running the real upstream exports; `markdownlint-cli2` passes on the new
