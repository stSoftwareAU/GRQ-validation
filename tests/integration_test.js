...
class IntegrationTest {
  ...
  testPortfolioTargetLogic() {
    ...
    console.log('Test Result:', Math.abs(portfolioTarget - individualTarget) < 0.1 ? 'PASS' : 'FAIL');
  ...
  testDividendExclusion() {
    ...
    const dividendsWithin90Days = testDividends.filter(d => d.exDivDate <= this.ninetyDayDate);
  ...
  testPerformanceCalculation() {
    ...
    console.log('Test Result:', Math.abs(totalReturn - 22.0) < 0.1 ? 'PASS' : 'FAIL');
  ...
  testChartDataLimitation() {
    ...
    const dataWithin90Days = mockDataPoints.filter(point => point.date <= this.ninetyDayDate);
    
    console.log('Total data points:', mockDataPoints.length);
    console.log('Data points within 90 days:', dataWithin90Days.length);
    console.log('Expected: 3 data points within 90 days (excluding score date and after 90 days)');
    console.log('Test Result:', dataWithin90Days.length === 3 ? 'PASS' : 'FAIL');
    
    return dataWithin90Days.length === 3;
  }

  runAllTests() {
    console.log('Running integration tests for 90-day portfolio calculations...\n');
    ...
    console.log('Overall Result:', test1 && test2 && test3 && test4 && test5 ? 'ALL TESTS PASSED' : 'SOME TESTS FAILED');
