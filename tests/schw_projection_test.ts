...
class TestGRQValidator {
  ...
  async loadDividendData() {
    const text = await Deno.readTextFile("docs/scores/2025/April/15-dividends.csv");
    const lines = text.trim().split("\n");
  ...
  getBuyPrice(stockSymbol: string, scoreDate: Date): { price: number; dateUsed: Date } | null {
    ...
    for (let i = 0; i <= 5; i++) {
      const targetDate = new Date(scoreDate.getTime() + i * 24 * 60 * 60 * 1000);
      const scoreData = marketData.find((point: MarketDataPoint) => {
        const pointDate = new Date(point.date.getFullYear(), point.date.getMonth(), point.date.getDate());
  ...
  getDividendsWithin90Days(stockSymbol: string): DividendDataPoint[] {
    ...
    return dividends.filter((div: DividendDataPoint) => div.exDivDate <= ninetyDayDate);
  }
  ...
  calculateStockPerformance(stock: StockData): number | null {
    ...
    const totalDividends = dividends.reduce((sum: number, div: DividendDataPoint) => sum + div.amount, 0);

    const totalReturn = (currentPrice - buyPrice + totalDividends) / buyPrice;
    return totalReturn * 100;
  }

  calculateTrendLine(stock: StockData, scoreDate: Date, endDate?: Date): TrendLineResult | null {
    ...
    marketData.forEach((point: MarketDataPoint) => {
      if (point.date >= scoreDate && point.date <= trendEndDate) {
        ...
        const dividends = this.getDividendsWithin90Days(stock.stock);
        const dividendsUpToDate = dividends.filter((d: DividendDataPoint) => d.exDivDate <= point.date);
        const totalDividends = dividendsUpToDate.reduce((sum: number, div: DividendDataPoint) => sum + div.amount, 0);
    ...
    const sumX = dataPoints.reduce((sum: number, point: DataPoint) => sum + point.x, 0);
    const sumY = dataPoints.reduce((sum: number, point: DataPoint) => sum + point.y, 0);
    const sumXY = dataPoints.reduce((sum: number, point: DataPoint) => sum + point.x * point.y, 0);
    const sumXX = dataPoints.reduce((sum: number, point: DataPoint) => sum + point.x * point.x, 0);
  ...
  calculateRSquared(dataPoints: DataPoint[], slope: number, intercept: number): number {
    const n = dataPoints.length;
    const meanY = dataPoints.reduce((sum: number, point: DataPoint) => sum + point.y, 0) / n;
  ...
  calculateTargetPercentage(stock: StockData, scoreDate: Date): number | null {
    ...
    const targetReturn = ((targetPrice - buyPriceObj.price) / buyPriceObj.price) * 100;
    return targetReturn;
  }

  // This is the actual function from app.js that we want to test
  calculateHybridProjection(stock: StockData, scoreDate: Date): HybridProjectionResult | null {
    const marketData = this.marketData[stock.stock];
...
Deno.test("NYSE:SCHW Projection Test - Using Real App Functions", async (t) => {
  ...
  const stock = validator.scoreData.find((s: StockData) => s.stock === "NYSE:SCHW");
  
  assert(stock, "NYSE:SCHW should be found in score data");

  await t.step("should calculate correct current performance using app functions", () => {
    ...
    assert(currentPerformance! > 15, `Current performance should be > 15%, got ${currentPerformance?.toFixed(1)}%`);
  });

  await t.step("should calculate correct 90-day projection using app functions", () => {
    const projection = validator.calculateHybridProjection(stock, scoreDate);
    assert(projection, "Should have projection");
    
    console.log(`Projected 90-day performance: ${projection.projected90DayPerformance.toFixed(1)}%`);
  ...
  await t.step("should have strong trend line fit using app functions", () => {
    ...
    console.log(`Trend line slope: ${trendLine.slope.toFixed(4)} (price change per day)`);
    
    // Should have good fit given the consistent upward trend
    assert(trendLine.rSquared > 0.3, `R-squared should be > 0.3, got ${trendLine.rSquared.toFixed(3)}`);
    
    // Slope should be positive (upward trend)
    assert(trendLine.slope > 0, `Slope should be positive, got ${trendLine.slope.toFixed(4)}`);
  });
}); 