...
Deno.test("parseCargoLock extracts package name/version pairs", () => {
  ...
  assertEquals(map.get("grq-validation"), "0.1.10");
  assertEquals(map.size, 3);
});

Deno.test("diffCargoLock ignores the workspace's own crate version bump", () => {
  // Regression for Issue #822: grq-validation is a path package with no
  // `source`, so its release-automation version bump has no upstream publish
  // time and must not be treated as an external dependency bump.
  const bumps = diffCargoLock(LOCK_OLD, LOCK_NEW);
  assert(
    !bumps.some((b) => b.name === "grq-validation"),
    "workspace-local packages must not be reported as bumps",
  );
});

Deno.test("diffCargoLock ignores newly-added path dependencies", () => {
  const withPathDep = LOCK_OLD + `
[[package]]
name = "local-helper"
version = "0.1.0"
`;
  assertEquals(diffCargoLock(LOCK_OLD, withPathDep).length, 0);
});
