...
class DividendCalculationTest {
  ...
  testDividendFiltering() {
    ...
    this.testDividends.forEach((div, index) => {
      ...
      console.log(
        `  ${index + 1}. ${div.exDivDate.toDateString()}: $${
          div.amount.toFixed(3)
        } ${isWithin90Days ? "(WITHIN 90 days)" : "(AFTER 90 days)"}`,
      );
    });
    ...
    console.log("");
    console.log(
      "Total dividends within 90 days: $" + totalDividends.toFixed(3),
    );
    console.log("Expected: $0.455 (0.135 + 0.32)");
    console.log("Actual: $" + totalDividends.toFixed(3));
    console.log(
      "Test Result:",
      Math.abs(totalDividends - 0.455) < 0.001 ? "PASS" : "FAIL",
    );

    return dividendsWithin90Days.length === 2 &&
      Math.abs(totalDividends - 0.455) < 0.001;
  }
