...
class PortfolioTargetTest {
  ...
  testPortfolioTargetCalculation() {
    ...
    console.log('Test Result:', Math.abs(calculatedTarget - expectedTarget) < 0.1 ? 'PASS' : 'FAIL');
  ...
  testPortfolioPerformance90Day() {
    ...
    this.mockPortfolio.forEach(stock => {
      ...
      const dividends = stock.dividends.reduce((sum, div) => sum + div.amount, 0);
      ...
      console.log(`  90-Day Price: $${mock90DayPrices[stock.stock].toFixed(2)}`);
      ...
      console.log(`  Return: ${((totalStockValue - investment) / investment * 100).toFixed(1)}%`);
      console.log('');
    });
    ...
    console.log('Test Result:', Math.abs(portfolioReturn - expectedReturn) < 2.0 ? 'PASS' : 'FAIL');
  ...
  testDividendInclusion() {
    ...
    const dividendsWithin90Days = testStock.dividends.filter(d => d.exDivDate <= this.ninetyDayDate);
    
    console.log('NYSE:WFG dividends within 90 days:', dividendsWithin90Days.length);
  ...
  runAllTests() {
    ...
    console.log('Overall Result:', test1 && test2 && test3 && test4 ? 'ALL TESTS PASSED' : 'SOME TESTS FAILED');
